...
/// Electron 本机 MCP 来源摘要（只读；字段兼容 camel / snake）。
struct AgentLocalMcpSource: Decodable, Hashable, Sendable {
    let kind: String
    let label: String
...
/// Agent 在在线 Electron 上已挂载且启用的 MCP 摘要（`GET .../local-mcp/attachments`）。
struct AgentLocalMcpAttachment: Decodable, Identifiable, Hashable, Sendable {
    let id: String
    let name: String
    let description: String
    ...
    let command: String?
    let enabled: Bool
    let source: AgentLocalMcpSource?

    enum CodingKeys: String, CodingKey {
        case id, name, description, url, command, enabled, source
        case transportKind
        case transportKindSnake = "transport_kind"
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        name = try c.decodeIfPresent(String.self, forKey: .name) ?? ""
        description = try c.decodeIfPresent(String.self, forKey: .description) ?? ""
        transportKind = try c.decodeIfPresent(String.self, forKey: .transportKind)
            ?? c.decodeIfPresent(String.self, forKey: .transportKindSnake)
            ?? ""
        url = try c.decodeIfPresent(String.self, forKey: .url)
        command = try c.decodeIfPresent(String.self, forKey: .command)
        enabled = try c.decodeIfPresent(Bool.self, forKey: .enabled) ?? true
        source = try c.decodeIfPresent(AgentLocalMcpSource.self, forKey: .source)
    }

    /// 品牌匹配用：HTTP 取 url，stdio 无 endpoint。
    var endpointForBrand: String? {
        let trimmed = url?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return trimmed.isEmpty ? nil : trimmed
    }
}

struct AgentLocalMcpAttachmentListResponse: Decodable, Sendable {
    let connections: [AgentLocalMcpAttachment]

    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        connections = try c.decodeIfPresent([AgentLocalMcpAttachment].self, forKey: .connections) ?? []
    }
